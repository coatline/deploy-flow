...
export default function Settings() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-surface-800 rounded p-4 space-y-3">
        {fields.map(f => (
          <div key={f.key}>
            ...
            <input
              ...
              className="w-full max-w-lg bg-surface-700 border border-surface-600 rounded px-3 py-1.5 text-sm"
            />
          </div>
        ))}
        <p className="text-xs text-surface-500">Sensitive fields are stored in your OS keychain.</p>
      </div>
    </div>
  )
}
